...
from .link_sender import (
    _get_caption, _media_type_of,
    send_preview_single, send_preview_album,
)
...
async def forward_topic_messages(client, src_entity, dst_entity,
    ...
    async def flush_pending_album():
        ...
        # ── Link mode: gửi PREVIEW album (thumbnail video/ảnh) + caption + link ──
        if link_mode:
            flush_skip_log(first_id - 1)
            try:
                sent_list = await send_preview_album(
    ...
    async for msg in iterator:
        ...
        # ── Link mode: gửi PREVIEW (thumbnail video/ảnh) + caption + link ────────
        if link_mode:
            try:
                flush_skip_log(msg.id - 1)
                sent = await send_preview_single(
...
async def run_session(client: TelegramClient, cfg: dict,
    ...
    async def flush_pending():
        ...
        if link_mode:
            # Gửi PREVIEW album + caption + link
            try:
                n = len(album)
                sent_list = await send_preview_album(
                    client, album, dst_entity,
                    dt if dt and dt != 1 else None, caption_tpl,
                )
                if sent_list:
    ...
    async for msg in client.iter_messages(src_entity, min_id=min_id, reverse=True):
        ...
        try:
            ...
            # ── Link mode: gửi PREVIEW + caption + link ──────────────────────────
